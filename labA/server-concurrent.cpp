...
#	endif
	...
	{	
		...
		for(size_t i = 0; i < connections.size(); ++i){     
			...
			bool state = connections[i].state;
			if(state == eConnStateReceiving){
				//printf("Adding socket %d to rset\n", connections[i].sock);
				FD_SET(connections[i].sock, &rset);
				if(connections[i].sock > listenfd)
					nfds = connections[i].sock;
			}
			if(state == eConnStateSending){
				//printf("Adding socket %d to wset\n", connections[i].sock);
		...
		int events = select(nfds+1,&rset, &wset,NULL,NULL);
		//printf("select() returned %d\n", events);  
		if(events == -1){
			perror("select encounted an error");
			continue;
		}
...
#			endif
		...
		for(size_t i = 0; i < connections.size(); i++){
			...
			if(FD_ISSET(connections[i].sock, &rset)){
